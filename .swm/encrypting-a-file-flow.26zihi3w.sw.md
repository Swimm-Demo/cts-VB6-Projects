...
---
This document describes how a file is encrypted for secure storage or transfer. The process checks file conditions, reads the file, encrypts its contents (optionally with a custom key), and writes the result as either raw bytes or base64-encoded data. Sensitive information is cleared after the operation.

```mermaid
flowchart TD
  node1["Starting the file encryption process"]:::HeadingStyle
  click node1 goToHeading "Starting the file encryption process"
  node1 --> node2["Encrypting the byte array with block cipher logic"]:::HeadingStyle
  click node2 goToHeading "Encrypting the byte array with block cipher logic"
  node2 --> node3{"Should output be base64-encoded?"}
  node3 -->|"Yes"| node4["Encoding the encrypted data as base64"]:::HeadingStyle
  click node4 goToHeading "Encoding the encrypted data as base64"
  node3 -->|"No"| node5["Writing the encrypted output file"]:::HeadingStyle
  click node5 goToHeading "Writing the encrypted output file"
...
In <SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="1555:4:4" line-data="Public Function EncryptFile(InFile As String, OutFile As String, Overwrite As Boolean, Optional Key As String, Optional OutputIn64 As Boolean) As Boolean">`EncryptFile`</SwmToken>, we set up the cipher state, then check if the input file exists and if we're allowed to overwrite the output file. If either check fails, we exit early.
...
<SwmSnippet path="/HotelManagementSystem/Modules/clsBlowfish.cls" line="506">

---

<SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="506:4:4" line-data="Private Sub Class_Initialize()">`Class_Initialize`</SwmToken> sets up the Blowfish cipher's P-array and S-boxes with the standard constants from the algorithm spec. These values are the foundation for all encryption and decryption, so if they're wrong or missing, nothing works. The function also sets a flag for runtime behavior, but the main point is getting the cipher state ready.
...
<SwmSnippet path="/HotelManagementSystem/Modules/clsBlowfish.cls" line="1561">

---

Back in <SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="1562:1:1" line-data="        EncryptFile = False">`EncryptFile`</SwmToken>, after setting up the cipher, we check if the output file exists and if overwriting is allowed. If not, we exit. If everything's good, we read the whole input file into a byte array (Buffer) so we can work on it in memory for encryption.
...
<SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="1619:4:4" line-data="Private Function FileExist(FilePath As String) As Boolean">`FileExist`</SwmToken> just tries to get the file length. If it works, the file exists. If it errors out, we know the file isn't there. Simple and direct.
...
<SwmSnippet path="/HotelManagementSystem/Modules/clsBlowfish.cls" line="1571">

---

After checking the files, <SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="1555:4:4" line-data="Public Function EncryptFile(InFile As String, OutFile As String, Overwrite As Boolean, Optional Key As String, Optional OutputIn64 As Boolean) As Boolean">`EncryptFile`</SwmToken> calls <SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="1571:3:3" line-data="    Call EncryptByte(Buffer(), Key)">`EncryptByte`</SwmToken> to actually encrypt the Buffer in memory using the provided key. This is where the file content gets scrambled.
...
## Encrypting the byte array with block cipher logic

```mermaid
%%{init: {"flowchart": {"defaultRenderer": "elk"}} }%%
flowchart TD
    node1["Prepare data for encryption"] --> node2{"Is a custom key provided?"}
    click node1 openCode "HotelManagementSystem/Modules/clsBlowfish.cls:348:354"
    node2 -->|"Yes"| node3["Set encryption key"]
    click node2 openCode "HotelManagementSystem/Modules/clsBlowfish.cls:350:350"
    node2 -->|"No"| node3
    click node3 openCode "HotelManagementSystem/Modules/clsBlowfish.cls:350:350"
    subgraph loop1["For each block in the data"]
      node3 --> node4["Writing encrypted blocks and reporting progress"]
      
    end
    node4 --> node5["Encryption complete; data is now protected"]
    click node5 openCode "HotelManagementSystem/Modules/clsBlowfish.cls:376:377"

classDef HeadingStyle fill:#777777,stroke:#333,stroke-width:2px;
click node4 goToHeading "Writing encrypted blocks and reporting progress"
node4:::HeadingStyle
...
%% flowchart TD
%%     node1["Prepare data for encryption"] --> node2{"Is a custom key provided?"}
%%     click node1 openCode "<SwmPath>[HotelManagementSystem/Modules/clsBlowfish.cls](HotelManagementSystem/Modules/clsBlowfish.cls)</SwmPath>:348:354"
%%     node2 -->|"Yes"| node3["Set encryption key"]
%%     click node2 openCode "<SwmPath>[HotelManagementSystem/Modules/clsBlowfish.cls](HotelManagementSystem/Modules/clsBlowfish.cls)</SwmPath>:350:350"
%%     node2 -->|"No"| node3
%%     click node3 openCode "<SwmPath>[HotelManagementSystem/Modules/clsBlowfish.cls](HotelManagementSystem/Modules/clsBlowfish.cls)</SwmPath>:350:350"
%%     subgraph loop1["For each block in the data"]
%%       node3 --> node4["Writing encrypted blocks and reporting progress"]
%%       
%%     end
...
In <SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="348:4:4" line-data="Public Sub EncryptByte(byteArray() As Byte, Optional Key As String)">`EncryptByte`</SwmToken>, we resize the byte array for padding, add random bytes and the original length, and set up for CBC mode. For each 8-byte block, we use <SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="361:3:3" line-data="        Call GetWord(LeftWord, byteArray(), Offset)">`GetWord`</SwmToken> to pull out two 32-bit words from the byte array, which are then processed for encryption. <SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="361:3:3" line-data="        Call GetWord(LeftWord, byteArray(), Offset)">`GetWord`</SwmToken> is needed to convert the byte data into the format the cipher expects.
...
<SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="428:6:6" line-data="Private Static Sub GetWord(LongValue As Long, CryptBuffer() As Byte, Offset As Long)">`GetWord`</SwmToken> pulls out 4 bytes from the buffer, reverses them, and packs them into a Long for the cipher.
...
After pulling out the 32-bit words with <SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="361:3:3" line-data="        Call GetWord(LeftWord, byteArray(), Offset)">`GetWord`</SwmToken>, <SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="348:4:4" line-data="Public Sub EncryptByte(byteArray() As Byte, Optional Key As String)">`EncryptByte`</SwmToken> calls <SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="365:3:3" line-data="        Call EncryptBlock(LeftWord, RightWord)">`EncryptBlock`</SwmToken> to run the Blowfish rounds on the block. This is the main encryption step for each chunk.
...
### Running the Blowfish encryption rounds

<SwmSnippet path="/HotelManagementSystem/Modules/clsBlowfish.cls" line="334">

---

<SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="334:6:6" line-data="Private Static Sub EncryptBlock(Xl As Long, Xr As Long)">`EncryptBlock`</SwmToken> runs the Blowfish rounds, alternating XORs and calls to f on the block halves. f is called to mix the data using the S-boxes, which is what makes the cipher strong. After all rounds, it swaps and finalizes the block with more XORs.
...
<SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="444:6:6" line-data="Private Static Function UnsignedAdd(ByVal Data1 As Long, Data2 As Long) As Long">`UnsignedAdd`</SwmToken> breaks two Longs into bytes, adds them with carry for each byte, and puts them back together. This is just to get unsigned addition working in VB6, which doesn't have it natively.
...
### Writing encrypted blocks and reporting progress
...
flowchart TD
    ...
    subgraph loop1["For each segment of data to encrypt"]
        node2["Process data segment"]
    ...
    loop1 --> node5{"Was 100% progress already reported?"}
    click node5 openCode "HotelManagementSystem/Modules/clsBlowfish.cls:376:376"
    node5 -->|"No"| node6["Report 100% completion"]
    click node6 openCode "HotelManagementSystem/Modules/clsBlowfish.cls:376:376"
    node5 -->|"Yes"| node7["Encryption complete"]
    click node7 openCode "HotelManagementSystem/Modules/clsBlowfish.cls:377:377"
classDef HeadingStyle fill:#777777,stroke:#333,stroke-width:2px;
...
%%     node5 -->|"Yes"| node7["Encryption complete"]
%%     click node7 openCode "<SwmPath>[HotelManagementSystem/Modules/clsBlowfish.cls](HotelManagementSystem/Modules/clsBlowfish.cls)</SwmPath>:377:377"
%% classDef HeadingStyle fill:#777777,stroke:#333,stroke-width:2px;
```
...
After <SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="334:6:6" line-data="Private Static Sub EncryptBlock(Xl As Long, Xr As Long)">`EncryptBlock`</SwmToken>, <SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="348:4:4" line-data="Public Sub EncryptByte(byteArray() As Byte, Optional Key As String)">`EncryptByte`</SwmToken> uses <SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="367:3:3" line-data="        Call PutWord(RightWord, byteArray(), Offset + 4)">`PutWord`</SwmToken> to write the encrypted words back into the byte array. It then updates the CBC state and fires progress events so the caller knows how far along we are. At the end, it makes sure to send a final 100% progress update.
...
<SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="436:6:6" line-data="Private Static Sub PutWord(LongValue As Long, CryptBuffer() As Byte, Offset As Long)">`PutWord`</SwmToken> takes a Long and writes its bytes into the buffer in reverse order, so the output is big-endian. This keeps the encrypted data compatible with the Blowfish format.
...
## Writing the encrypted output file
...
flowchart TD
    ...
    node2 -->|"No"| node4{"OutputIn64: Output as Base64?"}
    click node4 openCode "HotelManagementSystem/Modules/clsBlowfish.cls:1575:1575"
    node3 --> node4
    node4 -->|"Yes"| node5["Write Base64-encoded encrypted data to output file"]
    click node5 openCode "HotelManagementSystem/Modules/clsBlowfish.cls:1576:1576"
    node4 -->|"No"| node6["Write encrypted data to output file"]
    click node6 openCode "HotelManagementSystem/Modules/clsBlowfish.cls:1578:1578"
    node5 --> node7["Finish: Clean up sensitive data and return success"]
    click node7 openCode "HotelManagementSystem/Modules/clsBlowfish.cls:1580:1583"
    node6 --> node7
classDef HeadingStyle fill:#777777,stroke:#333,stroke-width:2px;
...
%%     node2 -->|"No"| node4{"<SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="1555:38:38" line-data="Public Function EncryptFile(InFile As String, OutFile As String, Overwrite As Boolean, Optional Key As String, Optional OutputIn64 As Boolean) As Boolean">`OutputIn64`</SwmToken>: Output as <SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="20:15:15" line-data="&#39; Standard Blowfish implementation with file support, Base64 conversion,">`Base64`</SwmToken>?"}
...
%%     node4 -->|"No"| node6["Write encrypted data to output file"]
%%     click node6 openCode "<SwmPath>[HotelManagementSystem/Modules/clsBlowfish.cls](HotelManagementSystem/Modules/clsBlowfish.cls)</SwmPath>:1578:1578"
%%     node5 --> node7["Finish: Clean up sensitive data and return success"]
%%     click node7 openCode "<SwmPath>[HotelManagementSystem/Modules/clsBlowfish.cls](HotelManagementSystem/Modules/clsBlowfish.cls)</SwmPath>:1580:1583"
%%     node6 --> node7
%% classDef HeadingStyle fill:#777777,stroke:#333,stroke-width:2px;
```

<SwmSnippet path="/HotelManagementSystem/Modules/clsBlowfish.cls" line="1572">

---

After <SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="348:4:4" line-data="Public Sub EncryptByte(byteArray() As Byte, Optional Key As String)">`EncryptByte`</SwmToken> finishes, <SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="1555:4:4" line-data="Public Function EncryptFile(InFile As String, OutFile As String, Overwrite As Boolean, Optional Key As String, Optional OutputIn64 As Boolean) As Boolean">`EncryptFile`</SwmToken> checks if the output file exists and deletes it if needed. Then it opens the file for binary writing and gets ready to write either raw encrypted bytes or base64, depending on <SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="1575:3:3" line-data="        If OutputIn64 = True Then">`OutputIn64`</SwmToken>.
...
<SwmSnippet path="/HotelManagementSystem/Modules/clsBlowfish.cls" line="1576">

---

After checking the output file, <SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="1581:1:1" line-data="    EncryptFile = True">`EncryptFile`</SwmToken> writes the encrypted data. If <SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="1555:38:38" line-data="Public Function EncryptFile(InFile As String, OutFile As String, Overwrite As Boolean, Optional Key As String, Optional OutputIn64 As Boolean) As Boolean">`OutputIn64`</SwmToken> is set, it calls <SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="1576:9:9" line-data="            Put #FileO, , EncodeArray64(Buffer())">`EncodeArray64`</SwmToken> to turn the bytes into base64 before writing. After writing, it cleans up all sensitive data and signals success.
...
# Encoding the encrypted data as base64
...
flowchart TD
    ...
    node1 -->|"Yes"| node3["Prepare for encoding"]
    click node3 openCode "HotelManagementSystem/Modules/clsBlowfish.cls:245:252"
    node2 --> node3
    node3 --> subgraph loop1["For each group of 3 bytes in input"]
        node4["Convert 3 bytes to 4 Base64 characters"]
        click node4 openCode "HotelManagementSystem/Modules/clsBlowfish.cls:260:266"
    end
    loop1 --> node5{"Bytes remaining at end?"}
    click node5 openCode "HotelManagementSystem/Modules/clsBlowfish.cls:268:284"
    node5 -->|"0 (No padding)"| node6["Finish encoding"]
    click node6 openCode "HotelManagementSystem/Modules/clsBlowfish.cls:285:286"
    node5 -->|"1 or 2 (Add = padding)"| node7["Add padding to output"]
    click node7 openCode "HotelManagementSystem/Modules/clsBlowfish.cls:268:284"
    node6 --> node8{"Is output <= max bytes per line?"}
    node7 --> node8
    click node8 openCode "HotelManagementSystem/Modules/clsBlowfish.cls:288:312"
    node8 -->|"Yes"| node9["Return Base64 string"]
    click node9 openCode "HotelManagementSystem/Modules/clsBlowfish.cls:289:290"
    node8 -->|"No"| subgraph loop2["For each line in output"]
        node10["Add line break"]
        click node10 openCode "HotelManagementSystem/Modules/clsBlowfish.cls:291:307"
    end
    loop2 --> node11["Return multi-line Base64 string"]
    click node11 openCode "HotelManagementSystem/Modules/clsBlowfish.cls:311:311"
...
%%     node1 -->|"Yes"| node3["Prepare for encoding"]
%%     click node3 openCode "<SwmPath>[HotelManagementSystem/Modules/clsBlowfish.cls](HotelManagementSystem/Modules/clsBlowfish.cls)</SwmPath>:245:252"
%%     node2 --> node3
%%     node3 --> subgraph loop1["For each group of 3 bytes in input"]
%%         node4["Convert 3 bytes to 4 <SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="20:15:15" line-data="&#39; Standard Blowfish implementation with file support, Base64 conversion,">`Base64`</SwmToken> characters"]
...
%%     node5 -->|"0 (No padding)"| node6["Finish encoding"]
%%     click node6 openCode "<SwmPath>[HotelManagementSystem/Modules/clsBlowfish.cls](HotelManagementSystem/Modules/clsBlowfish.cls)</SwmPath>:285:286"
%%     node5 -->|"1 or 2 (Add = padding)"| node7["Add padding to output"]
%%     click node7 openCode "<SwmPath>[HotelManagementSystem/Modules/clsBlowfish.cls](HotelManagementSystem/Modules/clsBlowfish.cls)</SwmPath>:268:284"
%%     node6 --> node8{"Is output <= max bytes per line?"}
%%     node7 --> node8
%%     click node8 openCode "<SwmPath>[HotelManagementSystem/Modules/clsBlowfish.cls](HotelManagementSystem/Modules/clsBlowfish.cls)</SwmPath>:288:312"
%%     node8 -->|"Yes"| node9["Return <SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="20:15:15" line-data="&#39; Standard Blowfish implementation with file support, Base64 conversion,">`Base64`</SwmToken> string"]
%%     click node9 openCode "<SwmPath>[HotelManagementSystem/Modules/clsBlowfish.cls](HotelManagementSystem/Modules/clsBlowfish.cls)</SwmPath>:289:290"
%%     node8 -->|"No"| subgraph loop2["For each line in output"]
%%         node10["Add line break"]
%%         click node10 openCode "<SwmPath>[HotelManagementSystem/Modules/clsBlowfish.cls](HotelManagementSystem/Modules/clsBlowfish.cls)</SwmPath>:291:307"
%%     end
%%     loop2 --> node11["Return multi-line <SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="20:15:15" line-data="&#39; Standard Blowfish implementation with file support, Base64 conversion,">`Base64`</SwmToken> string"]
%%     click node11 openCode "<SwmPath>[HotelManagementSystem/Modules/clsBlowfish.cls](HotelManagementSystem/Modules/clsBlowfish.cls)</SwmPath>:311:311"
%% 
%% classDef HeadingStyle fill:#777777,stroke:#333,stroke-width:2px;
```
...
<SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="56:4:4" line-data="Private Sub Initialize64()">`Initialize64`</SwmToken> fills <SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="57:1:1" line-data="    m_bytIndex(0) = 65 &#39;Asc(&quot;A&quot;)">`m_bytIndex`</SwmToken> with the ASCII codes for A-Z, a-z, 0-9, +, and /. It also sets up <SwmToken path="HotelManagementSystem/Modules/clsBlowfish.cls" pos="121:1:1" line-data="    m_bytReverseIndex(65) = 0 &#39;Asc(&quot;A&quot;)">`m_bytReverseIndex`</SwmToken> so we can map base64 characters back to their values. Both are needed for encoding and decoding.
